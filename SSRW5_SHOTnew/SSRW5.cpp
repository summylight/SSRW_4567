...
int main(int argc, char* argv[]){
    ...
    igraph_t graph[MAXSUBS],graph4[6];
...
//                        cout <<"find !!"<<nodes[i] <<' '<<nodes[j]<<endl;
                ...
                igraph_create(&subgraph4,&vc,0,IGRAPH_UNDIRECTED);
                igraph_bool_t bo;
                int whoisgraph4;
                for (whoisgraph4 = 0; whoisgraph4<6;whoisgraph4++){
                    igraph_isomorphic(&subgraph4,&graph4[whoisgraph4],&bo);
//                        cout << "iso result : " << ios <<endl;
                        if(bo){   //count isomorphic subgraph
