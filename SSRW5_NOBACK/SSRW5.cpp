...
void rmv(igraph_vector_t *vec , vector<long int > nodes , int pos){
...
int main(int argc, char* argv[]){
    ...
    if (argc <4){
        printf("The pramament of the procedure is like ./SSRW5C which_file_you_want_to_test times_you_want_to_walk jumpdistance repeat_time\n");
        exit(1);
    }

    char* filename = argv[1];
    long int given_time = atol(argv[2]);
    long int NEXTJUMP = atol(argv[3]);
    long int repeat_time = atoi(argv[4]);
